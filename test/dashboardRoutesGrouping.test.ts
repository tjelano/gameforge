...
describe('NavRail route grouping', () => {
  ...
  it('every DASHBOARD_ROUTES entry is either visible in one group or a known-hidden settings sub-route', () => {
    // The 6 individual /dashboard/settings/* pages are deliberately NOT
    ...
    // every route is either visible, or is one of those 6 known,
    // intentionally-hidden sub-routes. Asserting plain coverage against
    // the full DASHBOARD_ROUTES list (visible === all 18) would be wrong
    // by construction, since 6 of those 18 are never meant to be visible.
