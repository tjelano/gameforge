...
- Any client-side (`fetch(...)`) changes — **corrected after the task review caught this wrong**: this originally claimed "all 3 call sites live under `/dashboard`," which is false for `git/pull` — `app/login/LoginForm.tsx`'s "Pull from git first" button (shown only when zero accounts exist) calls it with no session, since a session always joins to a real user row. `app/api/git/pull/route.ts` needed a bootstrap exception (mirroring `app/api/auth/login/route.ts`'s own existing "allow account creation only while zero accounts exist" pattern) rather than a blanket guard. The other 6 routes' call sites are all under `/dashboard` as originally stated, already same-origin, no change needed there.
