...
// Only the creator may edit — enforced server-side in StyleService.update().
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const { requestingUserId, ...patch } = UpdateStyleSchema.parse(await req.json());
    const result = await styleService.update(id, requestingUserId, patch);
...
const DeleteStyleSchema = z.object({ requestingUserId: z.string().min(1) });

export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const { requestingUserId } = DeleteStyleSchema.parse(await req.json());
    ...
    if (existing.created_by !== requestingUserId) {
      return NextResponse.json({
        success: false,
